...
Based on vulnerability-db.json (as of 2026-03-12):
...
- **Vulnerability Documentation**:
  - [RCE Vulnerabilities](../vulnerabilities/rce.md)
  - [Command Injection](../vulnerabilities/command-injection.md)
  - [SSRF Vulnerabilities](../vulnerabilities/ssrf.md)
  - [Exec Bypass](../vulnerabilities/exec-bypass.md)
- **Best Practices**:
  - [Channel Security](../best-practices/channel-security.md)
  - [Hardening Guide - Gateway Security](../best-practices/hardening-guide.md#gateway-security)
- **Test Cases**: Available in repository `tests/agent-hijack/` directory
- **Official Documentation**: [OpenClaw Gateway Security](https://openclaw.ai/docs/gateway)
...
```bash
# Example: Run Test 1 (WebSocket RCE)
cd test-01-websocket-rce
./run.sh

# Check results
echo $?  # 0 = secure (RCE blocked), 1 = vulnerable
```
...
If tests uncover new vulnerabilities:

1. **Do not publicly disclose**: Contact security@openclaw.ai privately
2. **Provide details**:
   - OpenClaw version
   - Test case that succeeded
   - Attack payload used
   - Impact assessment (RCE severity)
3. **Allow time for fix**: Follow responsible disclosure (90 days)
4. **Coordinate CVE**: Work with OpenClaw security team for CVE assignment
