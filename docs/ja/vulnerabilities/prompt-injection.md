...
### hackerbot-claw GitHub Actionsキャンペーン（2026年）

AI搭載ボット「hackerbot-claw」（claude-opus-4-5ベースを自称）が、8日間（2月21〜28日）にわたりGitHub Actionsワークフローを自律的に攻撃。7リポジトリを標的とし、**5/7が侵害された**。aquasecurity/trivy（完全リポジトリ乗っ取り）、avelino/awesome-go（RCE + トークン窃取）が最も深刻。

キャンペーンで使われた5つのテクニック:
1. **Pwn Request**（`pull_request_target` + 非信頼checkout）— awesome-go, trivy
2. **直接スクリプトインジェクション** — project-akri/akri（CNCF）
3. **ブランチ名インジェクション**（bash `${{ }}` 展開）— microsoft/ai-discovery-agent, RustPython
4. **ファイル名インジェクション**（Base64エンコード）— DataDog/datadog-iac-scanner
5. **CLAUDE.md毒入れ**（AIプロンプトインジェクション）— ambient-code/platform

**重要な知見**: Attack 5はCLAUDE.mdを悪意ある指示に置換し、AIコードレビュアーを乗っ取る攻撃。Claude sonnet 4.6は2回の注入を即座に検出・拒否した。これは実環境で初めて文書化されたagent-vs-agent攻撃。

**OpenClawへの関連**: OpenClawもCLAUDE.mdを信頼コンテキストとして使うため、同一の毒入れリスクがある。詳細と防御チェックリストは[CI/CDサプライチェーン攻撃](cicd-supply-chain.md)を参照。

- **出典**: [StepSecurity Blog](https://www.stepsecurity.io/blog/hackerbot-claw-github-actions-exploitation)
...
あなたのOpenClawインスタンスがこれらの攻撃に対してどれだけ耐えられるか確認しませんか？テストガイドには、基本的な「前の指示を無視して」から高度なサプライチェーン攻撃やサイドチャンネル攻撃まで、29の実践的テストが含まれています。

**[プロンプトインジェクション テストガイド（29テスト）](../test-cases/prompt-injection-guide.md)**
